...
var H5ComponentPolyline_rank = function(name, cfg) {
	...
	var draw = function(per) {
		...
		//连线
		ctx.moveTo((w / (cfg.data.length + 1)), h - (cfg.data[0][1] / 848 * h * per));
		for(var i = 0; i < cfg.data.length; i++) {
			var item = cfg.data[i];
			x = (w / (cfg.data.length + 1)) * (i + 1);
			y = h - (item[1] / 848 * h * per);
		...
		for(var i = 0; i < cfg.data.length; i++) {
			var item = cfg.data[i];
			x = (w / (cfg.data.length + 1)) * (i + 1);
			y = h - (item[1] / 150 * h * per);

			ctx.font="20px Calibri";
			ctx.fillStyle = item[2] ? item[2] : '#595959';
			ctx.fillText((item[1] >> 0) , x - 10, y - 10);
		}
		ctx.stroke();
	}
